...
public static class SimplePool 
{
	...
	class Pool 
	{
		...
		public GameObject Spawn(Vector3 pos, Quaternion rot, bool spawnEnabled) 
		{
			GameObject obj;
			if(inactive.Count==0) 
			{
			...
			else 
			{
				...
				if(obj == null) 
				{
					...
					return Spawn(pos, rot, spawnEnabled);
				}
			}
			
			obj.transform.position = pos;
			obj.transform.rotation = rot;
			obj.SetActive(spawnEnabled);
			return obj;
	...
	static public GameObject Spawn(GameObject prefab, Vector2 pos, Quaternion rot, bool spawnEnabled) 
	{
		if(prefab==null) 
		{
			Debug.LogError("Trying to spawn a null prefab.");
			return null;
		}
		
		Init(prefab);
		
		return pools[prefab].Spawn(pos, rot, spawnEnabled);
	}
